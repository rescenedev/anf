...
- 소크: `ANF_SOAK=/Users/zihado/work swift run anfTests` — 루트 이하 **모든**
  디렉토리를 실제 읽기+정렬 경로로 순회, 최악 폴더 top10 출력. 기준선: 15만
  디렉토리 무실패, kr(26k) 외 전부 <55ms
- 대용량 복사: `ANF_BENCH_COPY=/폴더 swift run anfTests` — FileTransfer 실경로로
  임시 폴더에 복사(동일 볼륨 = APFS 클론), 시간·항목 수 검증 후 정리. 기준선:
...
- [ ] 분할 배치에서 ⌘1로 접은 뒤 보기 → "마지막 분할 배치 복원"으로 되살아남
      (백업 없으면 메뉴 비활성)
- [ ] 첫 실행에 단축키 카드 1회 표시, 시작하기/배경 클릭으로 닫힘, 이후 재시작에
      안 뜸, 보기 → "단축키 한눈에 보기"로 재호출
...
### 멀티윈도우 (아키텍처)
- [ ] ⌘N로 새 창, 각 창이 독립 탐색(폴더·탭·선택 공유 안 함), File → 새 창
- [ ] ⌘W로 탭 → 패널 → 창 순서로 닫힘, 마지막 창 닫으면 종료
- [ ] 창 반복 생성/파괴 누수 점검: `heap <pid> | grep 'WorkspaceModel\|AnfWindowController'` —
      AnfWindowController는 열린 창 수와 일치, 사이클 반복해도 증가하지 않을 것
      (WorkspaceModel은 직전 닫힌 창 때문에 +1까지는 정상=transient). footprint 안정.
      회귀: window over-release 크래시(isReleasedWhenClosed), 이벤트 모니터·옵저버 누수
- [ ] Get Info(⌘⌥I) 반복 열고 닫기 누수: `heap <pid> | grep GetInfoPanel` —
      닫은 뒤 라이브 인스턴스 0개. NSPanel도 isReleasedWhenClosed=false 필요
