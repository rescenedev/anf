...
- 파일 목록을 SwiftUI가 아닌 `NSTableView`로 렌더링 — 수만 개도 즉각 스크롤·정렬·이동
- 단일 · 2분할 좌우 · 상하 2행 · 4분할 레이아웃, 분할선 드래그, 레이아웃·보기 형태 기억
- 현재 레이아웃·탭 구성을 이름 붙여 저장하는 **Workspace**

**검색 · 미리보기**
- ⌘K 커맨드 팔레트: 파일명은 `fd`, 내용은 `ripgrep`, 백그라운드 인덱스 + 내장 퍼지 랭킹
- **hwpx·docx·pptx·xlsx 본문 검색** 및 인스펙터 텍스트 미리보기 (알한글 불필요)
- 커맨드 팔레트에서 SSH 호스트·Workspace 바로 점프
- 인스펙터 Quick Look + 가독성 텍스트 뷰어, ⌘± 폰트 크기

**원격 · 터미널**
- **GUI SFTP** — SSH 호스트를 일반 폴더처럼 탐색 (sshfs·macFUSE 불필요)
- 창 하단 전역 터미널 드로어, 실제 PTY, ⌘± 글자 크기, SSH 한 번에 연결
- `~/.ssh/config` 자동 인식 · 라이브 세션 표시

**편집 · 조작**
- 파인더식 인라인 이름 변경 (별도 팝업 없이 그 자리에서)
- 키보드 중심 단축키, 한글 입력 상태에서도 동작하는 ⌘ 단축키
- iCloud 미다운로드 인지, 복사·이동·휴지통·중복 등 기본 파일 작업
